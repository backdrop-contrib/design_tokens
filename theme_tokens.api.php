...
/**
 * @file
 * API documentation for Theme Tokens hooks.
 */

/**
 * Registers one or more token types with the Theme Tokens system.
 *
 * Sub-modules implement this hook to add support for a token type. The parent
 * module calls module_invoke_all('theme_tokens_types') when building the admin
 ...
 *
 * @see theme_tokens_get_types()
 * @see theme_tokens_build_field()
 */
function hook_theme_tokens_types() {
  return array(
    'color' => array(
      'label'          => t('Color'),
      'field callback' => 'mymodule_theme_tokens_build_color_field',
    ),
  );
}
 ...
 *
 * @see theme_tokens_load_info()
 */
function hook_theme_tokens_info_alter(array &$info, $theme_name) {
